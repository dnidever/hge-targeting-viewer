...
The app automatically downloads its default HGE catalog from the configured shared Google Drive file. The download and parsed catalog are cached for the life of the Streamlit server process. The sidebar uploader can replace the default with a FITS, CSV, Parquet, or whitespace-delimited table containing at least:
...
- The default Google Drive file must be shared as **Anyone with the link — Viewer**. Google Drive may rate-limit a heavily used public app; a dedicated data host is preferable if that becomes an issue.
